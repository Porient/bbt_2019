...
class JDCommentsCrawler:
    ...
    def save_csv(self,df,p):
        # 保存文件
        df.to_csv('jd_%d.csv'%p,encoding='gbk') 
    ...
    def crawler(self):
        ...
        for p in range(self.page):
            ...
            for com in comments:
                tmp_list.append([com['id'],productId,com['content']])
            df = pd.DataFrame(tmp_list,columns=['comment_id','product_id','content'])
            self.save_csv(df,p)
            dfs.append(df)
...
def merge_comment():
    product_ids = []
    contents = []
    df=pd.read_csv('jd_0.csv',sep=',',encoding="gbk")
    for product_id in df["product_id"]:
        product_ids.append(product_id)
    for content in df["content"]:
        contents.append(content)
    
    df=pd.read_csv('jd_1.csv',sep=',',encoding="gbk")
    for product_id in df["product_id"]:
        product_ids.append(product_id)
    for content in df["content"]:
        contents.append(content)

    df=pd.read_csv('jd_2.csv',sep=',',encoding="gbk")
    for product_id in df["product_id"]:
        product_ids.append(product_id)
    for content in df["content"]:
        contents.append(content)

    df=pd.read_csv('jd_3.csv',sep=',',encoding="gbk")
    for product_id in df["product_id"]:
        product_ids.append(product_id)
    for content in df["content"]:
        contents.append(content)

    df=pd.read_csv('jd_4.csv',sep=',',encoding="gbk")
    for product_id in df["product_id"]:
        product_ids.append(product_id)
    for content in df["content"]:
        contents.append(content)
    ...
    data.to_csv("6474492.csv",encoding="gbk")
